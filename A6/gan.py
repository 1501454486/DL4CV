...
def sample_noise(batch_size, noise_dim, dtype=torch.float, device="cpu"):
    ...
    # NOTE that rand is [0, 1), we need [-1, 1)
...
def generator_loss(logits_fake):
    ...
    # NOTE: ONES NOT ZEROS!!! loss = -E(log())
...
def build_dc_classifier():
    ...
    model = nn.Sequential(
        ...
        nn.LeakyReLU(0.01), nn.MaxPool2d(kernel_size = 2, stride = 2),
        nn.Conv2d(in_channels = 32, out_channels = 64, kernel_size = 5, stride = 1),
        nn.LeakyReLU(0.01), nn.MaxPool2d(kernel_size = 2, stride = 2),
        nn.Flatten(), nn.Linear(64 * 4 * 4, 64 * 4 * 4),
        nn.LeakyReLU(0.01), nn.Linear(64 * 4 * 4, 1)
    )
...
def build_dc_generator(noise_dim=NOISE_DIM):
    ...
    model = nn.Sequential(
        nn.Linear(noise_dim, 1024), nn.ReLU(),
        nn.BatchNorm1d(1024), nn.Linear(1024, 7 * 7 * 128), nn.ReLU(),
        nn.BatchNorm1d(7 * 7 * 128), nn.Unflatten(1, (128, 7, 7)),
