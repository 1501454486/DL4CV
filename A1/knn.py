...
from networkx import k_crust, number_of_cliques
import torch
...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for row in range(num_train):
        for col in range(num_test):
            dists[row, col] = sqrt(((x_train[row].reshape(-1) - x_test[col].reshape(-1)) ** 2).sum().item())
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
        dists[i] = torch.sqrt(((x_train[i].reshape(-1) - x_test.reshape(num_test, -1)) ** 2).sum(dim = 1))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    # Replace "pass" statement with your code
    # 重塑为 2D 矩阵
    x_train_flatten = x_train.reshape(num_train, -1)        # num_train x D
    x_test_flatten = x_test.reshape(num_test, -1)           # num_test x D

    # 计算 ||a||^2
    x_train_norm = (x_train_flatten ** 2).sum(dim = 1).reshape(-1, 1)

    # 计算 ||b||^2
    x_test_norm = (x_test_flatten ** 2).sum(dim = 1).reshape(1, -1)

    # 计算 -2a^Tb
    prod = -2 * torch.matmul(x_train_flatten, x_test_flatten.T)

    # 组合所有部分
    dists = torch.sqrt(x_train_norm + x_test_norm + prod)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    for col in range(num_test):
        # 获取距离最近的k个训练样本的索引
        _, indices = torch.topk(dists[:, col], k, largest = False)
        # 获取这k个样本对应的标签
        ...
        max_labels = torch.where(counts == max_count)[0]
        y_pred[col] = torch.min(max_labels)
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            ...
            k_to_accuracies[k].append(classifier.check_accuracy(x_valid, y_valid, k = k))
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    means = {k: (sum(v) / len(v)) for k, v in k_to_accuracies.items()}
